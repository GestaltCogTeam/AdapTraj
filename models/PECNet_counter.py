...
class PECNet_counter(nn.Module):
    ...
    def __init__(self, hyperparams):
        ...
        """
        super(PECNet_counter, self).__init__()
        ...
        self.encoder_dest = MLP(input_dim=2, output_dim=hyperparams.fdim, hidden_size=hyperparams.enc_dest_size)
        ...
        self.decoder = MLP(input_dim=hyperparams.fdim+hyperparams.zdim, output_dim=2, hidden_size=hyperparams.dec_size)

        self.non_local_theta = MLP(input_dim=2*hyperparams.fdim + 2, output_dim=hyperparams.non_local_dim, hidden_size=hyperparams.non_local_theta_size)
        self.non_local_phi = MLP(input_dim=2*hyperparams.fdim + 2, output_dim=hyperparams.non_local_dim, hidden_size=hyperparams.non_local_phi_size)
        self.non_local_g = MLP(input_dim=2*hyperparams.fdim + 2, output_dim=2*hyperparams.fdim + 2, hidden_size=hyperparams.non_local_g_size)

        self.predictor = MLP(input_dim=2*hyperparams.fdim + 2, output_dim=2*(hyperparams.pred_len-1), hidden_size=hyperparams.predictor_hidden_size)
    ...
    def forward(self, x, initial_pos, mask=None, dest=None):
        ...
        if self.training:
            ...
            prediction_features_c = torch.cat((ftraj_c, generated_dest_features, initial_pos), dim=1)
            for i in range(self.nonlocal_pools):
                # non local social pooling
                prediction_features_c = self.non_local_social_pooling(prediction_features_c, mask)
    ...
    def calculate_loss(self, x, reconstructed_x, mean, log_var, criterion, future, interpolated_future):
        # reconstruction loss
